...
如需使用其他设备配置，可通过 `LIDAR_SDK_CONFIG=./config/other_lidar.yaml bash scripts/start_lidar.sh` 指定配置文件；相对路径以当前终端目录解析。

本文档中的项目路径均以仓库根目录为基准。克隆后先在 WSL 终端运行：
...
后续命令使用 `./`、`ros2_ws/`、`bags/` 和 `exports/` 等相对路径，因此仓库可放在任意 Windows 或 WSL 目录。
...
ROS2 环境由 `scripts/start_lidar.sh`、`scripts/record_from_config.sh` 和 `scripts/export_points_from_bag.sh` 自动加载。默认使用 Jazzy；如 ROS 安装在非标准位置，可设置 `ROS_SETUP_FILE`，例如 `ROS_SETUP_FILE="$HOME/ros/jazzy/setup.bash"`。
...
```bash
./ros2_ws/src/rslidar_sdk
./ros2_ws/src/rslidar_msg
```
...
### `rslidar_sdk_config.yaml` 配置

配置文件位置：

```text
./config/rslidar_sdk_config.yaml
```

启动脚本默认读取此项目级配置；可在 Windows VSCode 或 WSL 中直接修改。它不会依赖子模块内的示例 `config.yaml`。
...
改完 `rslidar_sdk_config.yaml` 后，重启 SDK 生效：
...
```bash
source "${ROS_SETUP_FILE:-/opt/ros/${ROS_DISTRO:-jazzy}/setup.bash}"
...
### 2. 启动 SDK 节点，不启动 RViz

推荐通过项目脚本启动录制用的 SDK；脚本会按自身位置定位项目目录并传入项目级配置：
...
如需手动启动，先切换到仓库根目录，再执行：

```bash
source "${ROS_SETUP_FILE:-/opt/ros/${ROS_DISTRO:-jazzy}/setup.bash}"
source ./ros2_ws/install/setup.bash
ros2 run rslidar_sdk rslidar_sdk_node --ros-args -p config_path:=./config/rslidar_sdk_config.yaml
```
...
```bash
ls -td ./bags/points/points_* | head -1
```

查看 bag 信息：

```bash
ros2 bag info ./bags/points/points_具体时间
```
...
bash scripts/export_points_from_bag.sh \
  --input ./bags/points/points_具体时间 \
  --output ./exports/my_bin \
  --format bin
```
...
```bash
./bags/points/points_YYYYMMDD_HHMMSS
```
...
`bash scripts/start_lidar.sh` 只启动 SDK 节点；如需可视化，请在另一个终端运行 `rviz2`。
...
source "${ROS_SETUP_FILE:-/opt/ros/${ROS_DISTRO:-jazzy}/setup.bash}"
source ./ros2_ws/install/setup.bash
ros2 bag play ./bags/points/points_具体时间
```
...
```text
./exports/npy/points_具体时间
./exports/npz/points_具体时间
./exports/bin/points_具体时间
./exports/pcd/points_具体时间
```
...
同时 `rslidar_sdk_config.yaml` 中建议保持：
...
这是 shell `set -u` 和 ROS2 `setup.bash` 的兼容问题。当前脚本会在加载检测到的 ROS 环境前后处理，不需要手动修。
